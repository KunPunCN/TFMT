while getopts ':d:a:s:c:t:n:l:z:e:D:' opt
do
    case $opt in
        d)
        dataset="$OPTARG" ;;
...
precision=16
batch_size=4
data_dir="../data/cross_domain/${dataset}"
target_data_dir="../data/cross_domain/${dataset2}"

#${CUDA_IDS}
CUDA_VISIBLE_DEVICES=${CUDA_IDS} python3 train_cross.py \
  --gpus=1 \
  --precision=${precision} \
  --data_dir ${data_dir} \
