...
class F1_Measure:
    ...
    def cal_f1(self, pred_set, true_set):
        ...
        # pred_set = pred_set - intersection
        # true_set = true_set - intersection
        # #
        # pred_set = set((i[0], i[1][0:4]) for i in pred_set)
        # true_set = set((i[0], i[1][0:4]) for i in true_set)
        # intersection = pred_set.intersection(true_set)
        # sentie = len(intersection)
        # pred_set = pred_set - intersection
        # true_set = true_set - intersection
        # #
        # for i in pred_set:
...
class Result:
    ...
    @classmethod
    def parse_from_pair(cls, all_preds, examples):
        result_json = {}
        examples = {example['ID']: example for example in examples}
        ...
        for preds in all_preds:
            ...
            for ID in preds['ids']:
                ...
                for pp in example['pairs']:
                    # pl = polarity_map[pp[4]] + 1
                    # x = [pp[0],pp[1]]#,pp[2],pp[3]],pl
                    # x = [pp[2],pp[3]]
                    x = [pp[0], pp[1], pp[2], pp[3]]
                    # x = [pp[0], pp[1], pp[2], pp[3],pl]
                    # x = [pp[0], pp[1], pl]
                    # x = [pp[2], pp[3], pl]
                    if x not in pairs_true:
                        pairs_true.append(x)
                result_json[ID] = {
                    'ID': ID,
                    'sentence': example['sentence'],
            ...
            for (i, a_start, a_end, b_start, b_end, pol) in preds['all_preds']:#preds['pair_preds']:#
            # for (i, a_start, a_end) in preds['pair_preds']:
                ID = preds['ids'][i]
                result_json[ID]['all_preds'].add((a_start, a_end, b_start, b_end))
